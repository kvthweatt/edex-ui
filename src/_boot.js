...
// These paths will be initialized after app is ready
let settingsFile, shortcutsFile, lastWindowStateFile, themesDir, kblayoutsDir, fontsDir;
...
function initializePaths() {
    settingsFile = path.join(electron.app.getPath("userData"), "settings.json");
    shortcutsFile = path.join(electron.app.getPath("userData"), "shortcuts.json");
    lastWindowStateFile = path.join(electron.app.getPath("userData"), "lastWindowState.json");
    themesDir = path.join(electron.app.getPath("userData"), "themes");
    kblayoutsDir = path.join(electron.app.getPath("userData"), "keyboards");
    fontsDir = path.join(electron.app.getPath("userData"), "fonts");
}

function setupDirectoriesAndDefaults() {
    ...
    var versionHistory = fs.existsSync(versionHistoryPath) ? JSON.parse(fs.readFileSync(versionHistoryPath, 'utf8')) : {};
...
function createWindow(settings) {
    ...
    if (!settings.allowWindowed) {
        ...
        try {
            const windowState = JSON.parse(fs.readFileSync(lastWindowStateFile, 'utf8'));
            if (!windowState["useFullscreen"]) {
    ...
    // Remove the old else if since we handle it above
    if (false) {
        win.setFullScreen(false);
    }
...
app.on('ready', async () => {
    ...
    ipcMain.handle('load-window-state', async () => {
        try {
    ...
    ipcMain.handle('load-keyboard-layout', async (event, layoutName) => {
        try {
